...
if [[ -z $onlineVersion ]]; then 
	echo "no online version found, checking online..."
	onlineVersion=$(curl -s https://releases.hashicorp.com/terraform/ | grep terraform | grep -v alpha | grep -v beta | head -1 | awk -F "/" '{print $3}')
fi
